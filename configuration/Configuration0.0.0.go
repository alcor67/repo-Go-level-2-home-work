...
func Load() (*myConfig, error) {
	...
	//тестовая проверка загрузки переменной окружения
	//os.Setenv("MY_DIR", "testfolderempty")

	fmt.Println("os.Getenv(MY_DIR): ", os.Getenv("MY_DIR"))
	//читаем и проверяем переменные окружения для передачи флагов в тестах
	if varMyDir := os.Getenv("MY_DIR"); varMyDir != "" {
		conf.MyDir = varMyDir
	}

	if varMyDel, _ := strconv.ParseBool(os.Getenv("MY_DEL")); varMyDel {
		conf.MyDel = varMyDel
	}
